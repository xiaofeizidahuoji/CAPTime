...
class Exp_Short_Term_Forecast(Exp_Basic):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        loss_dist, pred = self.model(batch_x, batch_x_mark, batch_y, batch_y_mark)
                        loss_smape = criterion(batch_x, self.args.frequency_map, pred, batch_y, batch_y_mark)
                        loss = loss_dist + loss_smape
                else:
    ...
    def test(self, setting, test=0):
        ...
        smape_results, owa_results, mape, mase = m4_summary.evaluate(self.args.seasonal_patterns)
